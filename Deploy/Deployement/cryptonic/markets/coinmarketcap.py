...
class CoinMarketCap:
    ...
    def historic(cls, start='2013-04-28', stop=None, ticker='bitcoin', return_json=False):
        ...
        # url = f'{base_url}/{ticker}/historical-data/?start={start}&end={stop}'
        # r = requests.get(url)
        #
        ...
        client = Client(keys.Api() , keys.Secrets())
        timestamp = client._get_earliest_valid_timestamp('BTCUSDT', '1d')
        bars = client.get_historical_klines('BTCUSDT', '1d', timestamp, limit=1000 )
        btc_df = pd.DataFrame(bars, columns= ['date', 'open', 'high', 'low', 'close', 'volume',"close_time" ,'market_cap' , 'b' , 'c' , 'd' , 'e'])
        btc_df["date"] = pd.to_datetime(btc_df["date"], unit='ms')
        df = btc_df.drop(["close_time",'b' , 'c' , 'd' , 'e'] , axis = 1)
